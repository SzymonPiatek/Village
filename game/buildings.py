...
class Building:
    def __init__(self, name=None, resources=None, pos=None, resource_manager=None):
        self.config = configuration
        self.assets = assets
        self.name = name
        self.display_name = self.assets[name]["display_name"] if self.assets[name]["display_name"] else name
        self.description = self.assets[name]["description"] if self.assets[name]["description"] else None
        self.image = self.assets[name]["image"] if self.assets[name]["image"] else None
        self.rect = self.image.get_rect(topleft=pos) if self.image else None
        self.can_produce = False
        self.resource = self.assets[name]["resource"] if self.assets[name]["resource"] else None
        self.resources = resources
        self.resource_manager = resource_manager
        self.resource_manager.apply_cost_to_resource(self.name)
        self.resource_cooldown = pg.time.get_ticks() if self.resource else 0
        self.efficiency = self.assets[name]["efficiency"] if self.assets[name]["efficiency"] else None
